...
function manualSnapshot(q){
 const e=evidence[q.id];if(!e?.highlight)return null;const box=node('section',undefined,'manual-snapshot'),cap=node('div',undefined,'manual-snapshot-title');cap.innerHTML=`<span>FRAGMENTO DEL MANUAL</span><b>${e.unit} · PDF ${e.page}</b>`;const scroller=node('div',undefined,'manual-snapshot-scroll'),canvas=document.createElement('canvas');canvas.className='manual-snapshot-canvas';scroller.append(canvas);box.append(cap,scroller);const im=new Image();im.decoding='async';im.src=`content/${e.unit}/pages/${String(e.page).padStart(3,'0')}.webp`;im.onload=()=>{const h=e.highlight,cx=h.x+h.w/2,cy=h.y+h.h/2,cw=Math.min(1,Math.max(h.w+.06,.72)),ch=Math.min(1,Math.max(h.h+.09,.18)),x=Math.max(0,Math.min(1-cw,cx-cw/2)),y=Math.max(0,Math.min(1-ch,cy-ch/2)),sx=Math.round(x*im.naturalWidth),sy=Math.round(y*im.naturalHeight),sw=Math.max(1,Math.round(cw*im.naturalWidth)),sh=Math.max(1,Math.round(ch*im.naturalHeight));canvas.width=sw;canvas.height=sh;const ctx=canvas.getContext('2d');ctx.drawImage(im,sx,sy,sw,sh,0,0,sw,sh);const hx=(h.x-x)/cw*sw,hy=(h.y-y)/ch*sh,hw=h.w/cw*sw,hh=h.h/ch*sh;ctx.fillStyle='rgba(231,191,87,.14)';ctx.fillRect(hx,hy,hw,hh);ctx.strokeStyle='rgba(231,191,87,.92)';ctx.lineWidth=Math.max(3,sw*.004);ctx.strokeRect(hx,hy,hw,hh);requestAnimationFrame(()=>{scroller.scrollLeft=Math.max(0,(scroller.scrollWidth-scroller.clientWidth)/2);});};im.onerror=()=>box.classList.add('snapshot-error');return box;
}
const baseRenderFlashFace=renderFlashFace;
renderFlashFace=function(face,q,revealed=false){baseRenderFlashFace(face,q,revealed);if(revealed){const snap=manualSnapshot(q);if(snap){const question=face.querySelector('.question-text');face.insertBefore(snap,question);}}};
