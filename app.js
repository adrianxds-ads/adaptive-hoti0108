'use strict';
const APP_VERSION='2.2.0-activity-organizer',STORAGE_KEY='adaptive_hoti0108_v1',SOUND_KEY='adaptive_hoti0108_sound_v1',$=id=>document.getElementById(id),E=HotiQuiz;
...
function currentView(){return ['study','flash','conflicts','game','results','exam','examResults','statistics','setup','home'].find(id=>$(id)&&!$(id).hidden)||'home';}
function manualHref(q,variant=''){const e=evidence[q.id];if(!e)return '#';let src=e;if(variant==='conflict'&&e.conflictEvidence)src=e.conflictEvidence;else{const m=variant.match(/^additional(\d+)$/);if(m)src=e.additionalEvidence?.[Number(m[1])-1]||e;}const p=new URLSearchParams({evidence:q.id,returnView:currentView(),returnQ:q.id});if(variant)p.set('variant',variant);return `manuals.html?${p.toString()}#${src.unit||q.uf}/${src.page}`;}
function updateSoundButton(){if(!$('soundToggle'))return;$('soundToggle').textContent=soundOn?'🔊':'🔇';$('soundToggle').setAttribute('aria-label',soundOn?'Desactivar sonidos':'Activar sonidos');$('soundToggle').setAttribute('aria-pressed',String(soundOn));}
function toggleSound(){soundOn=!soundOn;localStorage.setItem(SOUND_KEY,soundOn?'on':'off');updateSoundButton();if(soundOn)signal('flip');}
function evidencePanel(q){const e=evidence[q.id];if(!e)return null;const verified=String(e.status||'').startsWith('verified_'),conflict=!verified,box=node('div',undefined,'evidence-card '+(conflict?'conflict':'verified'));let badge=verified?'✓ VERIFICADA EN MANUAL':e.status==='conflict_manual_internal'?'⚠ CONFLICTO INTERNO DEL MANUAL':'⚠ AMBIGÜEDAD DEL MANUAL';box.append(node('span',badge,'verification-badge'));const add=(src,label,variant='')=>{if(label)box.append(node('strong',label,'evidence-source-title'));box.append(node('blockquote',src.quote||'','evidence-quote'));box.append(node('p',`${src.unit||q.uf} · ${src.section||''} · PDF ${src.page}${src.printedPage?' · manual p. '+src.printedPage:''}`,'evidence-ref'));const a=node('a','Abrir fragmento en el manual ↗','evidence-link');a.href=manualHref(q,variant);box.append(a);};add(e,conflict?'Evidencia principal':null);if(e.conflictEvidence)add(e.conflictEvidence,e.status==='conflict_manual_internal'?'Segunda evidencia · contradicción':'Evidencia alternativa','conflict');(e.additionalEvidence||[]).forEach((src,i)=>add(src,'Continuación en el manual',`additional${i+1}`));return box;}
...
function flipFlashcard(){if(!flashList.length)return;if(!flashRevealed){flashRevealed=true;$('flashCard').classList.add('is-revealed');signal('flip');return;}if(flashIndex<flashList.length-1){flashIndex++;renderFlashcard();signal('next');$('flashCard').scrollIntoView({block:'start'});}}
...
function conflictQuestions(){return bank.filter(q=>{const e=evidence[q.id];return e&&!String(e.status||'').startsWith('verified_');});}
function renderConflicts(){show('conflicts');const rows=conflictQuestions(),host=$('conflictList');$('conflictCount').textContent=String(rows.length);$('conflictHomeCount').textContent=String(rows.length);host.replaceChildren();rows.forEach(q=>{const e=evidence[q.id],card=node('article',undefined,'conflict-card');card.dataset.questionId=q.id;card.append(node('p',metadata(q),'question-meta'),node('h2',q.question,'conflict-question'));const ans=node('div',undefined,'conflict-answer');ans.append(node('small','Respuesta usada para estudiar'),node('strong',`${q.correct_answer.toUpperCase()} · ${q.options[q.correct_answer]}`));card.append(ans,evidencePanel(q));host.append(card);});}
function restoreManualContext(){const p=new URLSearchParams(location.search),view=p.get('returnView'),qid=p.get('returnQ');if(!view)return;history.replaceState(null,'',location.pathname);const jump=(prefix)=>{const q=byId.get(qid);if(!q)return false;$(prefix+'Unit').value='all';populateAssessments(prefix);$(prefix+'Assessment').value='all';if(prefix==='study'){loadStudy();studyIndex=studyList.findIndex(x=>x.id===qid);renderStudy();}else{loadFlashcards();flashIndex=flashList.findIndex(x=>x.id===qid);renderFlashcard();}return true;};if(view==='flash'){show('flash');jump('flash');}else if(view==='study'){show('study');jump('study');}else if(view==='conflicts'){renderConflicts();requestAnimationFrame(()=>document.querySelector(`[data-question-id="${qid}"]`)?.scrollIntoView({block:'start'}));}else if(view==='game'&&state.active){resume();}else if(view==='results'&&state.lastResult){renderResults(state.lastResult);}else if(view==='exam'&&state.activeExam&&typeof resumeExam==='function'){resumeExam();}else if(view==='examResults'&&state.lastExam&&typeof renderExamResults==='function'){renderExamResults(state.lastExam);}else home();}
