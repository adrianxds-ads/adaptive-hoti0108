'use strict';
const APP_VERSION='1.4-exam-focus',STORAGE_KEY='adaptive_hoti0108_v1',$=id=>document.getElementById(id),E=HotiQuiz;
let bank=[],byId=new Map(),evidence={},root={},state={},canSave=true,studyList=[],studyIndex=0,session=null,tick=null,deadline=0,started=0,audioCtx=null;const titles={UF0080:'UF0080 · Organización del servicio',UF0081:'UF0081 · Gestión de la información',UF0082:'UF0082 · Atención al visitante'};
...
function show(id){if(typeof focusSwitch==='function')focusSwitch(id);if(id==='statistics'&&typeof renderExamHistory==='function')renderExamHistory();for(const name of ['home','study','setup','game','results','statistics','examSetup','exam','examResults'])$(name).hidden=name!==id;window.scrollTo(0,0);}
