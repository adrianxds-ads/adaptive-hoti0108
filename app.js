...
state={version:1,attempts:{},studyPositions:{},roundHistory:[],...((root.studyGame&&typeof root.studyGame==='object')?root.studyGame:{})};
if(!state.attempts||typeof state.attempts!=='object')state.attempts={};if(!state.studyPositions||typeof state.studyPositions!=='object')state.studyPositions={};if(!Array.isArray(state.roundHistory))state.roundHistory=[];
...
function show(id){for(const name of ['home','study','setup','game','results','statistics'])$(name).hidden=name!==id;window.scrollTo(0,0);}
...
function renderGame(){stopTimer();deadline=0;const q=byId.get(session.ids[session.index]);if(!q){finish();return;}renderPrompt($('gameCard'),q,true);$('feedback').hidden=true;$('nextQuestion').hidden=true;$('roundPosition').textContent=`${session.index+1} / ${session.ids.length}`;$('roundStreak').textContent=`Racha ${session.streak}`;$('roundProgress').max=session.ids.length;$('roundProgress').value=session.answers.length;const previous=session.answers[session.index];$('timer').hidden=!session.limit||!!previous;
...
function showFeedback(q,record,scroll){const feedback=$('feedback');feedback.replaceChildren();feedback.className='feedback '+record.kind;const heading=record.kind==='correct'?'CORRECTO':record.kind==='neutral'?'PREGUNTA CON MATIZ':record.answer===null?'TIEMPO AGOTADO':'INCORRECTO';feedback.append(node('h2',heading));if(record.answer&&record.kind!=='correct')feedback.append(node('p',`Tu respuesta: ${record.answer.toUpperCase()} · ${q.options[record.answer]}`,'feedback-user-answer'));if(record.kind==='neutral')feedback.append(node('p','Sin penalización ni cambio de racha.'));const reveal=node('div',undefined,'feedback-correct-answer');reveal.append(node('small',`Respuesta correcta · ${q.correct_answer.toUpperCase()}`),node('strong',q.options[q.correct_answer]));feedback.append(reveal);if(q.ambiguous)feedback.append(node('p',E.special(q),'special-note'));for(const o of $('gameCard').querySelectorAll('.option')){o.disabled=true;if(o.dataset.answer===q.correct_answer){o.classList.add('correct');o.querySelector('.option-text').append(node('span','✓ Respuesta correcta','option-badge'));}else if(o.dataset.answer===record.answer){o.classList.add(record.kind==='neutral'?'neutral':'wrong');o.querySelector('.option-text').append(node('span',record.kind==='neutral'?'Tu respuesta · sin penalización':'Tu respuesta','option-badge'));}else{o.classList.add('dim');}}feedback.hidden=false;$('nextQuestion').hidden=false;$('nextQuestion').textContent=session.index===session.ids.length-1?'Ver resultado →':'Siguiente pregunta →';$('roundStreak').textContent=`Racha ${session.streak}`;$('roundProgress').value=session.answers.length;$('timer').hidden=true;if(scroll){feedback.focus({preventScroll:true});feedback.scrollIntoView({block:'center',behavior:'instant'});}}
