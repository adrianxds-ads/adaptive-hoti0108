...
with sync_playwright() as w:
 ...
 page.goto(url);page.locator('#openStudy').wait_for();page.evaluate('localStorage.setItem("adaptive_hoti0108_v1", JSON.stringify({keepMe: "existing", studyGame: {version:1,totalPoints:0,attempts:{},studyPositions:{}}})); localStorage.setItem("adaptive_hoti_manual_reader_v1", "manual-progress");');page.reload();page.locator('#openStudy').wait_for();page.screenshot(path=str(OUT/'home.png'),full_page=True)
 ...
 assert stored()['studyGame']['totalPoints']==0;assert stored()['studyGame']['attempts']=={}
 ...
 assert float(page.locator('#studyQuestion').evaluate('(e)=>getComputedStyle(e).fontSize.replace("px", "")'))>=24
 assert float(page.locator('#studyCard .option').first.evaluate('(e)=>getComputedStyle(e).fontSize.replace("px", "")'))>=21
 ...
 for i in range(10):
  ...
  assert q['options'][q['correct_answer']] in page.locator('#feedback').text_content()
  if i==0:page.screenshot(path=str(OUT/'feedback.png'),full_page=True)
  page.locator('#nextQuestion').click()
 assert '7 de 10' in page.locator('#resultHeadline').text_content();assert stored()['studyGame']['totalPoints']==90;assert stored()['studyGame']['active'] is None;page.screenshot(path=str(OUT/'results.png'),full_page=True)
 ...
 seed(['UF0080_UD1_Q01','UF0080_UD1_Q02']);page.locator('#gameCard [data-answer="c"]').click();points=stored()['studyGame']['totalPoints'];page.locator('#pauseGame').click();page.reload();page.locator('#resumeGame').click();assert not page.locator('#feedback').is_hidden();assert stored()['studyGame']['totalPoints']==points;page.locator('#nextQuestion').click();assert page.locator('#gameCard').get_attribute('data-question-id')=='UF0080_UD1_Q02'
