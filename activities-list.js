...
function renderList(activities,sources,store){
 ...
 for(const [key,rows] of groups){
  ...
  for(const a of rows){   const src=sources.get(a.sequence),rec=store.records[a.sequence]||{status:'not_started'};
   ...
   b.addEventListener('click',()=>{location.href='./activity.html?activity='+encodeURIComponent(a.sequence)+'&v=312';});
   group.append(b);
  }
  host.append(group);
 }
...
async function init(){
 ...
 if(requested){location.replace('./activity.html?activity='+encodeURIComponent(requested)+'&v=312');return;}
